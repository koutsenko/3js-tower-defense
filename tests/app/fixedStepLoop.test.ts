...
describe('FixedStepLoop', () => {
  it('accumulates frame time into fixed gameplay steps and renders once', () => {
    ...
    expect(runtime.getSnapshot().simulationTime).toBeCloseTo(FIXED_STEP_SECONDS * 2);
  ...
  it('produces the same gameplay result for equivalent frame partitions', () => {
    ...
    const splitFrames = runFrames(Array.from({ length: 12 }, () => FIXED_STEP_SECONDS));
  ...
  it('caps a frame contribution at 250 milliseconds', () => {
    ...
    expect(advance).toHaveBeenCalledTimes(MAX_FRAME_DELTA_SECONDS / FIXED_STEP_SECONDS);
    expect(runtime.getSnapshot().simulationTime).toBeCloseTo(MAX_FRAME_DELTA_SECONDS);
  });
