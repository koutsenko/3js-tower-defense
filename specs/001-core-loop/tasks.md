...
**Причина блокировки:** утверждённый `plan.md` задаёт fixed-step execution и
порядок lifecycle → spawn → movement, но не определяет семантику публичного
`advance(deltaSeconds)` при пересечении одним вызовом границы начала волны или
нескольких границ spawn schedule. Применение полного `deltaSeconds` ко всем
созданным в таком вызове monsters учитывает время до их появления и нарушает
скорость `1 cell/s`; разбиение вызова на внутренние интервалы либо расчёт
индивидуального active time требует предварительно утверждённого изменения
technical design.
