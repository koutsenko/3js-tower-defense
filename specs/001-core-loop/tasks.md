...
**Причина блокировки:** не определено, как `advance(deltaSeconds)` должен
обрабатывать пересечение lifecycle и spawn boundaries без начисления monsters
времени до их появления. Требуется утверждение
